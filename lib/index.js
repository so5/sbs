...
class SBS extends EventEmitter {
  ...
  getResult(id) {
    ...
    if (this.failed.hasOwnProperty(id)) {
      rt = this.failed[id];
      if (!this.noAutoClear) this.failed[id] = null;
    } else if (this.finished.hasOwnProperty(id)) {
      rt = this.finished[id];
      if (!this.noAutoClear) this.finished[id] = null;
    }
    return rt;
  }
   ...
   */
  clearResults() {
    for (const id in this.failed) {
      this.failed[id] = null;
    }
    for (const id in this.finished) {
      this.finished[id] = null;
    }
  }
  ...
  async _executer() {
    ...
    this.running.add(id);
    try {
      this.finished[id] = await exec(args);
    } catch (err) {
      if (retry(err)) {
