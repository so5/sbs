...
class SBS extends EventEmitter {
  ...
  getResult(id, keepResult) {
    ...
    if (this.failed.has(id)) {
      ...
      if (!this.noAutoClear && !keepResult) this.failed.set(id, null);
    } else if (this.finished.has(id)) {
      rt = this.finished.get(id);
      if (!this.noAutoClear && !keepResult) this.finished.set(id, null);
    }
    return rt;
  }
  ...
  async qwait(id, keepResult) {
    ...
    if (state === "finished") {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (!p) p = [];
        p.push({ resolve: resolve, reject: reject, keepResult: keepResult });
